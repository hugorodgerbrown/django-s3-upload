...
def get_endpoint(region):
    """Return S3 endpoint based on the configured region.configured

    See http://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region

    """
    return 's3.amazonaws.com' if region == 'us-east-1' else 's3-%s.amazonaws.com' % region
...
def create_upload_data(
    ...
    # default region is us-east-1 as that defaults to root s3.amazon.com
    region = getattr(settings, 'S3UPLOAD_REGION', 'us-east-1')
...
def get_signed_download_url(
    ...
    key_obj = get_key_from_url(parse.unquote(source_url), bucket_name)
...
def remove_signature(url):
    ...
    query_vars = parse.parse_qs(parsed_url.query)
    ...
    if len(query_vars) > 0:
        return "{0}?{1}".format(
            address_only,
            parse.urlencode(query_vars, doseq=True)
        )
